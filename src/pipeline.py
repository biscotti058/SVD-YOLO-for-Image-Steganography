...
class StegoPipeline:
    def __init__(self, alpha=0.01, mode='svd_yolo', yolo_model='yolov8n.pt', yolo_confidence=0.25):
        self.alpha = alpha
        self.mode = mode
        self.stego = SVDSteganography(alpha=alpha)
        if mode == 'svd_yolo':
            self.yolo = YOLORegionSelector(model_name=yolo_model, confidence=yolo_confidence)
        else:
            self.yolo = None
        self.mask = None
    ...
    def encode(self, cover_path, secret_path, output_dir='output'):
        ...
        self.metrics = {
            'psnr': psnr, 'ssim': ssim, 'alpha': self.alpha, 'mode': self.mode,
            'cover_shape': cover.shape, 'secret_shape': secret.shape,
        ...
        return {'stego_path': stego_path, 'keys_path': keys_path, 'metrics': self.metrics}

    def decode(self, stego_path, keys_path, output_dir='output'):
        os.makedirs(output_dir, exist_ok=True)
    ...
    def compare_modes(self, cover_path, secret_path, alphas=None, output_dir='output'):
        if alphas is None:
        ...
        for mode in modes:
            for alpha in alphas:
                ...
                try:
                    ...
                    recovered = pipeline.decode(result['stego_path'], result['keys_path'], sub_dir)
                    ...
                    print(f"  Error: {e}")
                    all_results.append({'metrics': {'mode': mode, 'alpha': alpha, 'error': str(e)}})
        return all_results

    def evaluate_dataset(self, cover_dir, secret_path, output_dir='output/dataset_results'):
        os.makedirs(output_dir, exist_ok=True)
        ...
        for path in tqdm(cover_paths, desc=f"Valutazione {self.mode}"):
            ...
            try:
                ...
                res = self.encode(path, secret_path, os.path.join(output_dir, f"{self.mode}_{filename}_temp"))
                recovered = self.decode(res['stego_path'], res['keys_path'], os.path.join(output_dir, f"{self.mode}_{filename}_temp"))
